...
variable "node_pools" {
  type = map(object({
    ...
    # instance_store_policy controls how local NVMe instance-store volumes are
    # used. Set to "RAID0" to format and mount them as the underlying filesystem
    # for kubelet and containerd (faster node ephemeral-storage; allocatable
    # ephemeral-storage becomes the instance-store size). Leave null to ignore
    # instance-store volumes. See
